...
async function getDataImpl(path: string, body?: unknown): Promise<any> {
  ...
  try {
    ...
    if (cached) {
      window.$ErrorLogger.addBreadcrumb(
        "api-cache",
        `${npath} served from offline cache`,
        { path: npath },
      );
...
export async function login(
  ...
  const messageRef = showMessage(
    "loading",
    i18n.global.t("ui.messages.loading"),
    {
      duration: 6000,
    },
  );
  ...
  try {
    ...
    const cached = readApiCache<ResultOf<Users["Authenticate"]>>(
      cacheKey,
      requestBody,
    );
    messageRef.destroy();
    if (cached) {
      window.$ErrorLogger.addBreadcrumb(
        "api-cache",
        `${cacheKey} served from offline cache`,
        {},
      );
      return cached;
    }
